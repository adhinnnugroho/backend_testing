<x-app-layout>
    <div class="px-6 py-6">
        <div class="flex items-center justify-between mb-4">
            <h1 class="text-2xl font-light text-gray-700">Edit Karyawan</h1>
        ...
        <div class="bg-white shadow-sm border border-gray-200 rounded-sm p-6">
            <form method="POST" action="{{ route('employees.update', $employee) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input name="name_employee" value="{{ old('name_employee', $employee->name_employee) }}"
                ...
                <div>
                    ...
                    <select name="position_id" class="mt-1 block w-full rounded-sm border-gray-300 text-sm">
                        ...
                        @foreach ($positions as $pos)
                            <option value="{{ $pos->id }}" @selected(old('position_id', $employee->position_id) == $pos->id)>{{ $pos->position_name }}
                            </option>
                        @endforeach
                    </select>
                    @error('position_id')
                ...
                <div>
                    ...
                    <select name="home_town_id" class="mt-1 block w-full rounded-sm border-gray-300 text-sm">
                        ...
                        @foreach ($hometowns as $ht)
                            <option value="{{ $ht->id }}" @selected(old('home_town_id', $employee->home_town_id) == $ht->id)>{{ $ht->home_town_name }}
                            </option>
                        @endforeach
                    </select>
                    @error('home_town_id')
                ...
                <div>
                    ...
                    <input type="date" name="date_of_birth"
                        value="{{ old('date_of_birth', \Illuminate\Support\Carbon::parse($employee->date_of_birth)->format('Y-m-d')) }}"
                        class="mt-1 block w-full rounded-sm border-gray-300 text-sm" />
                    @error('date_of_birth')
                        <div class="text-red-600 text-xs mt-1">{{ $message }}</div>
                    @enderror
                ...
                <div class="flex justify-end space-x-2">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm bg-blue-500 text-white rounded-sm">Perbarui</button>
                </div>
            </form>
        </div>
    </div>
